<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-gray-800">Register New Rescued Cat</h2>
    </x-slot>

    <div class="bg-white rounded-xl shadow-sm p-6 max-w-4xl">
        <div class="mb-6">
            <h3 class="font-semibold text-gray-800 text-lg">Intake Information</h3>
            <p class="text-sm text-gray-500">Complete the profile for the new arrival to begin tracking care.</p>
        </div>
        ...
        <form method="POST" action="{{ route('cats.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cat Name <span class="text-red-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Luna" required
                ...
                <div>
                    ...
                    <input type="text" name="breed" value="{{ old('breed') }}" placeholder="e.g. Domestic Shorthair"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="text" name="age_estimate" value="{{ old('age_estimate') }}" placeholder="e.g. 2 years"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        <option value="rescued" @selected(old('status', 'rescued') === 'rescued')>New Intake</option>
                        <option value="in_treatment" @selected(old('status') === 'in_treatment')>In Treatment</option>
                        <option value="ready_for_adoption" @selected(old('status') === 'ready_for_adoption')>Ready for Adoption</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="color" value="{{ old('color') }}" placeholder="e.g. Orange Tabby"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Rescue Location</label>
                    <input type="text" name="rescue_location" value="{{ old('rescue_location') }}" placeholder="e.g. Jl. Merdeka No. 10"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Catatan tambahan tentang kucing ini..."
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-red-500 mt-1">⚠ Photo is highly recommended for adoption listings.</p>
            </div>
